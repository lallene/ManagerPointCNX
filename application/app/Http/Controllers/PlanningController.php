  ...
  public function index(Request $request): View
{
    ...
    $me = Agent::with('projets.site')
        ->where('work_email', $email)
        ->firstOrFail();

    $mySiteIds = $me->projets
        ->pluck('site_id')
        ->unique()
        ->filter()
        ->toArray();

    $myProjetIds = $me->projets
        ->pluck('id')
        ->toArray();
    ...
    if (!$isFullAccess) {
        if ($user->hasAnyRole(['Top Manager', 'Top Formateur', 'Top CQ', 'Top Superviseur'])) {
            ...
            if ($user->hasRole('Top Manager')) {
                ...
                // Top Manager : uniquement ses projets
                $query->whereIn('ap.projet_id', $myProjetIds);

                // Catégories visibles pour Top Manager
                $query->where(function ($q) {
                    $q->where('a.fonction', 'LIKE', 'FORMATEUR%')
                      ->orWhere('a.fonction', 'LIKE', 'CONTRÔLEUR%')
                      ->orWhere('a.fonction', 'LIKE', 'CONTROLEUR%')
                      ->orWhere('a.fonction', 'LIKE', 'SUPERVISEUR%');
                });

            } elseif ($user->hasRole('Top Formateur')) {

                // Top Formateur : formateurs de son site
                $query->whereIn('p.site_id', $mySiteIds)
                    ->where('a.fonction', 'LIKE', 'FORMATEUR%');

            } elseif ($user->hasRole('Top CQ')) {

                // Top CQ : contrôleurs qualité de son site
                $query->whereIn('p.site_id', $mySiteIds)
                    ->where(function ($q) {
                        $q->where('a.fonction', 'LIKE', 'CONTRÔLEUR%')
                          ->orWhere('a.fonction', 'LIKE', 'CONTROLEUR%')
                          ->orWhere('a.fonction', 'LIKE', 'CQ%');
                    });

            } elseif ($user->hasRole('Top Superviseur')) {

                // Top Superviseur : superviseurs de ses projets
                $query->whereIn('ap.projet_id', $myProjetIds)
                    ->where('a.fonction', 'LIKE', 'SUPERVISEUR%');
            }

        } else {

            // Manager simple : uniquement ses projets
            $query->whereIn('ap.projet_id', $myProjetIds);
        }
    }
    ...
    $agents = $agents->filter(function ($agent) use ($fonctionsChoisies) {
        return in_array($agent->fonction, $fonctionsChoisies);
    });
    ...
    $plannings = DB::table('plannings')
        ...
        ->get()
        ->map(function ($item) {
            $item->entree = $item->entree
                ? \Carbon\Carbon::parse($item->entree)->format('H:i')
                : null;

            $item->sortie = $item->sortie
                ? \Carbon\Carbon::parse($item->sortie)->format('H:i')
                : null;
    ...
    public function getPlanningData(Request $request): JsonResponse
    {
        try {
            ...
            if ($request->filled('site_id')) {
                $queryProjets->where('site_id', $request->get('site_id'));
            }
            ...
            $projets = $queryProjets
                ->with([
                    'agents' => function ($q) use ($dates, $user, $isFullAccess) {
                        ...
                        $q->with(['plannings' => fn($pq) => $pq->whereIn('jour', $dates)])->with(['pointages' => fn($ptq) => $ptq->whereIn('date_pointage', $dates)]);
                    },
                ])
                ->get();
                ...
                foreach ($projet->agents as $agent) {
                    ...
                    foreach ($dates as $date) {
                        ...
                        $statsParDate[$date] = [
                            ...
                            'status' => $status,
                            'retard' =>
                                $pt && $p && Carbon::parse($pt->entree)->gt(Carbon::parse($p->entree)->addMinutes(5))
                                    ? Carbon::parse($pt->entree)
                                        ->diff(Carbon::parse($p->entree))
                                        ->format('%H:%I')
                                    : null,
                        ];
                    }
    ...
    public function getDailyPlanningData(Request $request): JsonResponse
    {
        try {
            ...
            $queryProjets = Projet::with([
                'agents' => function ($q) use ($date, $user, $isFullAccess) {
                    $q->whereHas('user', fn($uq) => $uq->role('Manager'));
                    ...
                    $q->with(['plannings' => fn($pq) => $pq->whereDate('jour', $date)])->with(['pointages' => fn($ptq) => $ptq->whereDate('date_pointage', $date)]);
                },
            ]);
            ...
            foreach ($projets as $projet) {
                ...
                foreach ($projet->agents as $agent) {
                    ...
                    if (!$planning) {
                        continue;
                    } // On n'affiche que les agents planifiés ce jour-là
    ...
    {
        $semaines = [];
        $start = Carbon::now()->subWeeks($minus);
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $topManager = Agent::with('projets')->where('work_email', $authUser->work_email)->firstOrFail();
        ...
        try {
            ...
            foreach ($request->plannings as $agentId => $jours) {
                ...
                if ($authUser->hasRole('Top Manager') && !empty(array_intersect($allowedProjetIds, $agent->projets->pluck('id')->toArray()))) {
                    $canModify = true;
                }

                // 2. Top Formateur : peut modifier les formateurs de son site
                if ($authUser->hasRole('Top Formateur') && $agent->site_id === $topManager->site_id && str_starts_with($agentFonction, 'formateur')) {
                    $canModify = true;
                }

                // 3. Top CQ : peut modifier les contrôleurs qualité de son site
                if ($authUser->hasRole('Top CQ') && $agent->site_id === $topManager->site_id && (str_starts_with($agentFonction, 'cq') || str_starts_with($agentFonction, 'controlleur qualité') || str_starts_with($agentFonction, 'controleur qualité') || str_starts_with($agentFonction, 'contrôleur qualité'))) {
                    $canModify = true;
                }
            ...
            return back()->with('success', "Planning de la semaine {$request->week} enregistré avec succès.");
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function import(Request $request)
    {
        ...
        if ($import->successCount > 0) {
            return back()->with('success', "Importation réussie : {$import->successCount} planning(s) de Manager(s) créé(s) ou mis à jour.");
        }
    ...
    public function pasteImport(Request $request)
    {
        ...
        $currentAgent = Agent::with('projets')->where('work_email', $user->work_email)->first();

        if (!$isAdmin && !$currentAgent) {
            return back()->with('error', "Profil agent introuvable pour l'email : {$user->work_email}");
        }

        $allowedProjetIds = $currentAgent ? $currentAgent->projets->pluck('id')->toArray() : [];
        ...
        foreach ($lines as $index => $line) {
            ...
            if ($index === 0 && isset($columns[0], $columns[1]) && (str_contains(strtolower($columns[0]), 'id') || str_contains(strtolower($columns[1]), 'date'))) {
                continue;
            }
            ...
            try {
                ...
                if ($isTopManager && $targetAgent->user->hasRole('Manager') && !empty(array_intersect($allowedProjetIds, $targetAgent->projets->pluck('id')->toArray()))) {
                    $canImport = true;
                }

                // Top Formateur : uniquement les Formateurs de son site
                if ($isTopFormateur && $targetAgent->site_id === $currentAgent->site_id && ($targetAgent->user->hasRole('Formateur') || str_starts_with($targetFonction, 'formateur'))) {
                    $canImport = true;
                }

                // Top CQ : uniquement les CQ / Contrôleurs qualité de son site
                if ($isTopCQ && $targetAgent->site_id === $currentAgent->site_id && ($targetAgent->user->hasRole('CQ') || str_starts_with($targetFonction, 'cq') || str_starts_with($targetFonction, 'controlleur qualité') || str_starts_with($targetFonction, 'controleur qualité') || str_starts_with($targetFonction, 'contrôleur qualité'))) {
                    $canImport = true;
                }
                ...
                try {
                    $dateObj = str_contains($rawDate, '/') ? \Carbon\Carbon::createFromFormat('d/m/Y', $rawDate) : \Carbon\Carbon::parse($rawDate);
                } catch (\Exception $e) {
                ...
                $entree = isset($columns[2]) && strtoupper(trim($columns[2])) !== 'OFF' ? trim($columns[2]) : null;

                $sortie = isset($columns[3]) && strtoupper(trim($columns[3])) !== 'OFF' ? trim($columns[3]) : null;
